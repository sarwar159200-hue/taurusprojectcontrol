...
This package is the complete v12 weighted S-curve release for the Taurus Project Control portal. It uses Next.js, Vercel and Supabase.
...
- Interactive S-curves for all available years at year, month and project-week level on both the Executive Overview and Progress pages.
- Exact workbook discipline/sub-discipline filters, cumulative curves, reporting-point SPI and a paginated full schedule table.
- Controlled weighted aggregation for Overall, Engineering, Procurement and Construction totals using the approved Taurus weight table.
- Responsive no-scroll S-curves with every month visible and monotone weekly smoothing through the exact workbook points.
...
If migrations 0001–0004 are already installed, run migrations 0005 and 0006. Both are idempotent. Migration 0006 stores document, progress and schedule rows in reliable batches and independently tracks the latest progress and schedule versions.
...
## B. Replace the GitHub repository contents

The expected repository layout is:

```text
taurusprojectcontrol/
...
Upload the extracted `taurusprojectcontrol-scurve-v12` folder to the GitHub repository root. Do not upload a duplicate wrapper inside it. Do not upload Excel, XER, `.env` or `.env.local` files.

## C. Configure Vercel

Set the Vercel **Root Directory** to `taurusprojectcontrol-scurve-v12`.
...
Apply each variable to Production, Preview and Development, then redeploy.
...
The portal validates, analyzes and publishes replacement XLSX progress/MDR and schedule files that retain the supplied workbook formats. Compact dashboard summaries and version pointers are stored in `published_project_updates`; document, cumulative progress and schedule rows are written to their normalized version tables in small batches. Document hyperlinks are read from MDR column D. Monthly disciplines are Overall, Engineering, Procurement, Construction and Mobilization. Weekly curves are available for Engineering and Construction. Their exact workbook sub-disciplines are normalized to readable labels. Every S-curve uses cumulative values, every monthly period receives a visible label, and SPI/SV align planned and actual at the latest actual reporting point. Project weeks use the controlled project calendar: Week 29 begins 15-Jan-2026 and 05-Feb-2026 is Week 32. Binary workbook storage in OneDrive and native Primavera XER logic are not included in this release.
