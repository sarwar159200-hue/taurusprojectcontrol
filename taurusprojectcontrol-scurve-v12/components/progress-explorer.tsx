...
export function ProgressExplorer({ series, fallbackCurve, variant = "full" }: {
  ...
  const weightedSeries = useMemo(() => applyProgressWeights(series), [series]);

  const sourceFrequency = grain === "weekly" ? "weekly" : "monthly";
  const frequencyPoints = weightedSeries.filter((point) => point.frequency === sourceFrequency);
  ...
  const displayedCurve = weightedSeries.length ? curve : fallbackForGrain;
  ...
  const structure = useMemo(() => {
    ...
    for (const point of weightedSeries) {
...
function ProgressControls({ disciplines, discipline, setDiscipline, subdisciplines, subdiscipline, setSubdiscipline, years, year, setYear }: {
  ...
  const weightText = (value: string) => {
    const weight = progressWeight(discipline, value);
    if (weight === null) return null;
    const decimals = weight * 100 % 1 === 0 ? 0 : 2;
