...
import { applyProgressWeights, makeProgressCurve } from "../progress-analytics";
import {
  dateOf,
  increment,
  normalize,
...
function measureFromLabel(label: string): ProgressSeriesPoint["measure"] | null {
  ...
  if (normalized.includes("planned") || /(^|\s)plan(\s|$)/.test(normalized)) return "planned";
...
function disciplineFromLabel(label: string) {
  const cleaned = label
    .replace(/\b(?:baseline|planned|plan|actual|forecast)\b/gi, "")
    .replace(/\b(?:cumulative|monthly|weekly|progress)\b/gi, "")
    .replace(/\bdate\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();
...
function readProgressSeries(workbook: Workbook, monthlyActualCutoff: string | null) {
  ...
  for (const sheet of workbook.worksheets) {
    ...
    const sheetActualCutoff = frequency === "monthly"
      ? monthlyActualCutoff
      : area === "Engineering"
        ? lastActualDate(sheet, 11, 18)
        : area === "Construction"
          ? lastActualDate(sheet, 18, 32)
          : monthlyActualCutoff;
    ...
    for (let rowNumber = 1; rowNumber <= sheet.rowCount; rowNumber += 1) {
      ...
      const subject = disciplineFromLabel(label);
      const isAreaTotal = subject === "Overall";
      const discipline = canonicalDiscipline(area === "Overall" ? subject : area);
      const subdiscipline = area === "Overall" || isAreaTotal
        ? null
        : canonicalSubdiscipline(subject);
      const activeMeasure: ProgressSeriesPoint["measure"] = currentMeasure;
      let running = 0;
      dates.forEach((periodDate, index) => {
        if (!periodDate) return;
        if (activeMeasure === "actual" && sheetActualCutoff && periodDate > sheetActualCutoff) return;
        const value = numberOf(row.getCell(index + 2));
        if (value === null) return;
        const key = [frequency, discipline, subdiscipline ?? "", activeMeasure, periodDate].join("|");
...
export function importProgressWorkbook(
  ...
  const workbookChart = overall ? readOverallCurve(overall, actualCutoff) : [];
  const progressSeries = applyProgressWeights(readProgressSeries(workbook, actualCutoff));
  const weightedOverall = makeProgressCurve(
    progressSeries.filter((point) => point.frequency === "monthly" && point.discipline === "Overall" && point.subdiscipline === null),
    "monthly",
    "all"
  );
  const chart = weightedOverall.length ? weightedOverall : workbookChart;
