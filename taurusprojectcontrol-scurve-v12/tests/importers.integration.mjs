...
const structureUrl = pathToFileURL(new URL("../lib/progress-structure.ts", import.meta.url).pathname).href;
const { PROGRESS_WEIGHTS } = await import(structureUrl);
...
for (const [main, weights] of Object.entries(PROGRESS_WEIGHTS)) {
  const totalWeight = Object.values(weights).reduce((sum, value) => sum + value, 0);
  const expectedTotal = main === "Construction" ? 0.9999 : 1;
  assert(Math.abs(totalWeight - expectedTotal) < 0.0000001, `${main} weights changed from the controlled values.`);
}

function assertWeightedTotal(frequency, discipline, measure, date) {
  const weights = PROGRESS_WEIGHTS[discipline];
  const total = progress.progressSeries.find((point) =>
    point.frequency === frequency && point.discipline === discipline && point.subdiscipline === null &&
    point.measure === measure && point.periodDate === date
  );
  assert(total, `${frequency} ${discipline} total is missing for ${date}.`);
  const expected = Object.entries(weights).reduce((sum, [subdiscipline, weight]) => {
    const child = progress.progressSeries.find((point) =>
      point.frequency === frequency && point.discipline === discipline && point.subdiscipline === subdiscipline &&
      point.measure === measure && point.periodDate === date
    );
    return sum + (child?.cumulativeValue ?? 0) * weight;
  }, 0);
  assert(Math.abs(total.cumulativeValue - expected) < 0.0000001, `${discipline} total must use controlled weights.`);
}

assertWeightedTotal("monthly", "Engineering", "actual", "2026-06-01");
assertWeightedTotal("monthly", "Procurement", "actual", "2026-06-01");
assertWeightedTotal("weekly", "Engineering", "actual", "2026-07-09");
assertWeightedTotal("weekly", "Construction", "actual", "2026-07-09");
