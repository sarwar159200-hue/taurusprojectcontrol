...
export async function sendSecurityAlertEmail(input: {
  ...
  const eventLabel = input.event === "login" ? "User login" : "New user created";
  const now = new Date().toISOString();
  const subject = `[Taurus Security] ${eventLabel}: ${input.actorEmail}`;
  const text = [
    "Taurus Project Control security notification",
    "",
    `Event: ${eventLabel}`,
    `User: ${input.actorName || "Not provided"}`,
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, to: SECURITY_ALERT_RECIPIENTS, subject, text }),
    cache: "no-store"
  });
  if (!response.ok) throw new Error(`Security email returned HTTP ${response.status}.`);
