...
const EUR_TO_USD = 1.15; // Management-approved fixed conversion: 1 EUR = 1.15 USD
...
type PaymentSnapshot = {
  ...
  projectTotals: { USD: number; EUR: number; PLN: number };
  currencyNote: string;
};
...
function supplierCurrentValues(workbook: ExcelJS.Workbook) {
  ...
  // Current values come from each supplier schedule AFTER REVISION, converted on the
  // management-approved basis: EUR/USD = 1.15 and workbook PLN/USD where applicable.
  if (ge) current.GE = result(ge.getCell("L4")) * eurUsd + result(ge.getCell("M4")) + result(ge.getCell("N4")) * plnUsd;
  if (spg) current.SPG = result(spg.getCell("M4")) * eurUsd + result(spg.getCell("N4")) + result(spg.getCell("O4")) * plnUsd;
  if (jc) current.JC = result(jc.getCell("L4")) * eurUsd + result(jc.getCell("M4")) + result(jc.getCell("N4")) * plnUsd;
...
function parseWorkbook(workbook: ExcelJS.Workbook, fileName: string, userEmail: string): PaymentSnapshot {
  ...
  ipa.eachRow((row, rn) => {
    ...
    if (supplier && supplier in supplierPaid) {
      // Recalculate EUR rows at the management-approved fixed FX rate.
      // USD rows remain native USD; PLN keeps the workbook's controlled USD equivalent.
      const eur = result(row.getCell(6));
      const pln = result(row.getCell(7));
      const usd = result(row.getCell(8));
      const workbookUsdEquivalent = result(row.getCell(9));
      const controlledUsd = eur ? eur * EUR_TO_USD : (pln ? workbookUsdEquivalent : (usd || workbookUsdEquivalent));
      supplierPaid[supplier] += controlledUsd;
    }
  });
  workbook.worksheets.filter((s) => /^\d+$/.test(s.name.trim())).forEach((ws) => {
    ws.eachRow((row, rn) => {
      ...
      const currency = norm(row.getCell(4).value);
      const sourceAmount = result(row.getCell(5));
      const workbookUsd = result(row.getCell(8)) || result(row.getCell(7));
      supplierPaid[supplier] += (currency === "euro" || currency === "eur") && sourceAmount
        ? sourceAmount * EUR_TO_USD
        : workbookUsd;
    });
  });
  ...
  ipa.eachRow((row, rn) => {
    ...
    variations.push({ voNo: `VO-${String(vo).padStart(3, "0")}`, supplier: matched?.supplier || "Project / Unassigned", description: matched?.desc || `Approved variation recorded in IPA row ${rn}`, amount, currency, amountUsd: currency.trim().toUpperCase() === "USD" ? amount : (["EUR", "EURO"].includes(currency.trim().toUpperCase()) ? amount * EUR_TO_USD : (matched?.usd ?? null)), status: "Approved", approvedDate: date });
  });

  // Reconcile supplier-level contract adjustments that are evidenced by the controlled
  // supplier schedules but are not separately identified in the IPA approved-VO ledger.
  // In this workbook SPG is USD 22.05m original (Price Breakdown B9+B10) versus
  // USD 21.75m current (ITEM 2 - SPGDRY(ACC)!P4), i.e. a -USD 300,000 adjustment.
  for (const s of suppliers) {
    if (Math.abs(s.variations) < 0.5) continue;
    const represented = variations.some((v) => v.supplier === s.supplier && Math.abs((v.amountUsd ?? v.amount) - s.variations) < 1);
    if (!represented) variations.push({
      voNo: `ADJ-${s.supplier}`, supplier: s.supplier,
      description: `${s.supplier} contract-value adjustment reconciled from supplier schedule`,
      amount: s.variations, currency: "USD", amountUsd: s.variations,
      status: "Contract Adjustment", approvedDate: null
    });
  }

  // Overall project total requested by Cost Control: sum the IPA column
  // "Payment Application Amount (USD)". Do not mix EUR/PLN into this KPI.
  // We sum the detail rows directly instead of relying on a cached Excel formula,
  // so server-side analysis is deterministic on Vercel.
  let paymentApplicationUsdColumn = 0;
  ipa.getRow(1).eachCell((cell, col) => {
    if (norm(cell.value) === "payment application amount usd") paymentApplicationUsdColumn = col;
  });
  let paymentApplicationUsdTotal = 0;
  if (paymentApplicationUsdColumn) {
    ipa.eachRow((row, rn) => {
      if (rn <= 1) return;
      const first = norm(row.getCell(1).value);
      const fourth = norm(row.getCell(4).value);
      if (first === "total" || fourth === "total") return;
      paymentApplicationUsdTotal += result(row.getCell(paymentApplicationUsdColumn));
    });
  }
  const projectTotals = { USD: paymentApplicationUsdTotal, EUR: 0, PLN: 0 };
  ...
  return {
    ...
    suppliers, variations, projectTotals,
    currencyNote: "Main KPIs follow the IPA USD contract-price ledger (AQ) and cumulative paid equivalent USD (AF). EUR values are converted at the fixed management rate 1 EUR = 1.15 USD. PLN remains on the workbook source conversion. Supplier current values use the workbook's controlled USD-equivalent totals where provided."
  };
}
...
export async function POST(request: Request) {
  ...
  try {
    const arrayBuffer = await file.arrayBuffer(); const workbook = new ExcelJS.Workbook(); await workbook.xlsx.load(arrayBuffer as any);
    ...
    if (error) throw new Error(`Payment data could not be saved to Supabase: ${error.message}${error.details ? ` — ${error.details}` : ""}${error.hint ? ` — ${error.hint}` : ""}`);
    ...
    const message = error instanceof Error
      ? error.message
      : (error && typeof error === "object" && "message" in error ? String((error as { message?: unknown }).message) : "Payment workbook could not be analyzed.");
    const url = new URL("/dashboard/payments", request.url);
    url.searchParams.set("paymentError", message);
...
export async function GET() {
  ...
  dash.addRow([]);
  dash.addRow(["Total Payment Application Amount (USD)", money(snapshot.projectTotals?.USD ?? 0)]);
  dash.getRow(13).font = { bold: true, color: { argb: "FF0B2B4B" } };
  dash.addRow([]); dash.addRow(["Supplier", "Original Value", "Variations", "Current Value", "Paid", "Remaining", "Payment %"]); styleHeader(dash.getRow(15));
