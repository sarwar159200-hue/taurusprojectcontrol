...
export default async function PaymentsPage({ searchParams }: { searchParams?: Promise<{ uploaded?: string; paymentError?: string; supplier?: string }> }) {
  ...
  // Normalize legacy stored snapshots at render time so the dashboard remains correct
  // even before the workbook is republished after this upgrade.
  const displayVariations: Variation[] = snapshot ? snapshot.variations.map((v) => {
    const currency = (v.currency || "").trim().toUpperCase();
    const amountUsd = (currency === "EUR" || currency === "EURO") ? v.amount * 1.15 : (currency === "USD" ? v.amount : v.amountUsd);
    return { ...v, amountUsd };
  }) : [];
  if (snapshot) {
    for (const s of snapshot.suppliers) {
      if (Math.abs(s.variations) < 0.5) continue;
      const alreadyRepresented = displayVariations.some((v) => v.supplier === s.supplier && Math.abs((v.amountUsd ?? v.amount) - s.variations) < 1);
      if (!alreadyRepresented) displayVariations.push({
  ...
  const filteredVariations = displayVariations.filter((v) => supplierFilter === "All" || v.supplier === supplierFilter);
  ...
  return <div className="payment-page payment-reference-layout">
    ...
    {snapshot ? <section className="project-contract-total project-contract-total-single" aria-label="Total payment application amount in USD"><div><span>Total Payment Application Amount (USD)</span><small>Sum of Payment Application Amount (USD) from the controlled IPA worksheet</small></div><strong>$ {fmt(projectTotals.USD)}</strong></section> : null}
