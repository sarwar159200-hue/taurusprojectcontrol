# Taurus Project Control v18 — start here
...
Upload the extracted `taurusprojectcontrol-professional-v18` folder to the root of the GitHub repository.

GitHub must contain this exact file path:

`taurusprojectcontrol-professional-v18/package.json`

Do not upload the outer ZIP/download folder, Excel files, `.env` files or passwords.

## 2. Run the three final Supabase repairs
...
5. Run the complete `supabase/migrations/0007_document_control_command_center.sql` file.
6. Confirm migration 0007 returns the document fields `due_date`, `responsible_party` and `review_cycle_days`.

All three migrations are safe to rerun. Migration 0006 replaces the oversized single-row publish with reliable batched version storage. Migration 0007 adds the MDR contractual workflow fields required by the Document Control Command Center.

## 3. Confirm Vercel

Exact Root Directory for that GitHub layout:

`taurusprojectcontrol-professional-v18`
