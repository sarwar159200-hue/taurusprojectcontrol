...
async function loadDocumentsDirect(versionId: string): Promise<DocumentRecordInput[]> {
...
async function loadProgressSeriesDirect(versionId: string): Promise<ProgressSeriesPoint[]> {
  const supabase = createAdminClient();
...
async function loadScheduleActivitiesDirect(versionId: string): Promise<ScheduleActivityInput[]> {
  const supabase = createAdminClient();
  const rows = await fetchVersionRows(
    supabase,
    "schedule_activities",
...
// Published version rows never change after a successful publish. Cache them by
// version id so moving between dashboard tabs does not repeatedly download and
// transform thousands of rows from Supabase. A new publish creates a new version
// id, therefore users receive the new data immediately without stale-cache risk.
function loadDocuments(versionId: string) {
  return unstable_cache(
    () => loadDocumentsDirect(versionId),
...
function loadProgressSeries(versionId: string) {
  return unstable_cache(
    () => loadProgressSeriesDirect(versionId),
...
function loadScheduleActivities(versionId: string) {
  return unstable_cache(
    () => loadScheduleActivitiesDirect(versionId),
...
export async function getPublishedProjectUpdate(
  ...
  if (progressAnalysis && options.documents && data.progress_version_id) {
    jobs.push(loadDocuments(data.progress_version_id).then((rows) => { progressAnalysis.documents = rows; }));
  }
  if (progressAnalysis && options.progressSeries && data.progress_version_id) {
    jobs.push(loadProgressSeries(data.progress_version_id).then((rows) => { progressAnalysis.progressSeries = rows; }));
  }
  if (scheduleAnalysis && options.scheduleActivities && data.schedule_version_id) {
    jobs.push(loadScheduleActivities(data.schedule_version_id).then((rows) => { scheduleAnalysis.scheduleActivities = rows; }));
  }
