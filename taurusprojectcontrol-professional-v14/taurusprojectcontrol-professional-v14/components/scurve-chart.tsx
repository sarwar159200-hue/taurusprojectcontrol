...
export function ScurveChart({ points, granularity = "monthly" }: { points: CurvePoint[]; granularity?: "year" | "monthly" | "weekly" }) {
  ...
  const pointSpacing = granularity === "weekly" ? 86 : 92;
  const width = Math.max(960, Math.max(0, prepared.length - 1) * pointSpacing + 124);
  const height = granularity === "weekly" ? 470 : 438;
  const left = 62;
  const right = 30;
  const top = 30;
  ...
  return (
    <div className="chart-wrap professional-scurve" aria-label={t("Project S-curve")}>
      <div className="chart-scroll-shell" tabIndex={0} aria-label={t("Scroll horizontally to view every reporting period")}>
      <svg style={{ width: `${width}px`, minWidth: `${width}px` }} viewBox={`0 0 ${width} ${height}`} role="img">
        <defs>
        ...
        {prepared.map((point, index) => (
          <text
            ...
            x={x(index)}
            y={height - (granularity === "weekly" ? 48 : 30)}
            textAnchor={granularity === "weekly" ? "end" : "middle"}
            transform={granularity === "weekly" ? `rotate(-42 ${x(index)} ${height - 48})` : undefined}
            className="chart-axis-label chart-period-label"
          >
            {new Date(`${point.date}T00:00:00Z`).toLocaleDateString(dateLocale, {
      ...
      {prepared.length > 10 ? <div className="chart-scroll-note"><span>↔</span>{t("All reporting periods are labelled")}</div> : null}
