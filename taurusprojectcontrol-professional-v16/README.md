...
- Exact workbook discipline/sub-discipline filters, professional monotone cumulative curves, baseline-only reporting-point SPI/SV, expected-finish forecast and a paginated full schedule table.
...
- Controlled project reporting-week number plus date on every weekly S-curve label (`05-Feb-2026 = W32`).
...
```text
taurusprojectcontrol-professional-v16/
├── app/
├── components/
├── lib/
├── public/
...
Upload the extracted `taurusprojectcontrol-professional-v16` folder. Remove the old folder only after v16 deploys successfully. Do not upload Excel, XER, `.env` or `.env.local` files.
