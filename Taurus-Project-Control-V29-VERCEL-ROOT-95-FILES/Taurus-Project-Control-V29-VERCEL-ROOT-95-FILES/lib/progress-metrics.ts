...
export function scheduleSignal(spi: number | null): ScheduleSignal {
  ...
  if (spi < 0.99) return "ahead";
  if (spi <= 1.01) return "on-plan";
  if (spi <= 1.04) return "slightly-behind";
  return "delayed";
}

/**
 * Baseline-only schedule forecast:
 * SPI = baseline cumulative / actual cumulative at the same data date.
 * SV (%) = actual cumulative - baseline cumulative.
 * With this project-specific inverse SPI convention, expected duration = approved baseline duration × SPI.
 */
export function progressPerformance(source: CurvePoint[]): ProgressPerformance {
  ...
  const spi = actual !== null && actual > 0 && baseline !== null ? baseline / actual : null;
