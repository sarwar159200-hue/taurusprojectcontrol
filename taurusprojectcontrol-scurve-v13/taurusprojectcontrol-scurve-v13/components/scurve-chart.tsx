...
function tickIndexes(length: number, maximum = 6) {
  ...
  return (
    <div className="chart-wrap professional-scurve" aria-label={t("Project S-curve")}>
      <svg viewBox={`0 0 ${width} ${height}`} role="img">
        ...
        {tickIndexes(prepared.length).map((index) => (
          <text key={index} x={x(index)} y={height - 25} textAnchor="middle" className="chart-axis-label">
            {granularity === "year"
              ? prepared[index]?.date.slice(0, 4)
              : new Date(`${prepared[index]?.date}T00:00:00Z`).toLocaleDateString(dateLocale, {
