...
- ISO week number plus date on every weekly S-curve reporting label.
...
```text
taurusprojectcontrol-professional-v15/
├── app/
├── components/
├── lib/
├── public/
...
Upload the extracted `taurusprojectcontrol-professional-v15` folder. Remove the old folder only after v15 deploys successfully. Do not upload Excel, XER, `.env` or `.env.local` files.
