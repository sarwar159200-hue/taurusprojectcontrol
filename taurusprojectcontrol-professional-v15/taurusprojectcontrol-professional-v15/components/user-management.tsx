  ...
  return (
    <details className="user-card">
      <summary>
        ...
        <span className={`status-pill ${user.isActive ? "status-ready" : "status-error"}`}>{user.isActive ? t("Active") : t("Disabled")}</span>
        <span className="user-role-label">{t(roleOptions.find((option) => option.value === user.role)?.label ?? user.role.replaceAll("_", " "))}</span>
        <span className="user-login"><small>{t("Last login")}</small><strong>{formatDate(user.lastLoginAt, dateLocale, t("Never"))}</strong></span>
      </summary>
    ...
    <div className="user-management-stack">
      ...
      <section className="panel">
        <div className="panel-heading">
          ...
          {!canManage ? <span className="status-pill status-loading">{t("View only")}</span> : null}
        </div>
