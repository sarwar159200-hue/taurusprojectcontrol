...
export function importProgressWorkbook(
  ...
  const greatestActualDate = progressSeries
    ...
    // A future period containing a saved zero is not considered "reached".
    // Use the greatest exact date where actual progress is genuinely positive.
    .filter((point) =>
      (point.incrementalValue !== null && point.incrementalValue > 0)
      || (point.cumulativeValue !== null && point.cumulativeValue > 0)
    )
