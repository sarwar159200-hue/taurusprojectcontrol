...
export function scheduleSignal(spi: number | null): ScheduleSignal {
  ...
  // Management control thresholds:
  // SPI > 1.00       = ahead of schedule (green)
  // SPI 0.98–1.00    = on schedule / acceptable tolerance (green)
  // SPI 0.95–<0.98   = slightly behind schedule (yellow)
  // SPI < 0.95       = behind schedule (red)
  if (spi > 1.00) return "ahead";
  if (spi >= 0.98) return "on-plan";
  if (spi >= 0.95) return "slightly-behind";
  return "delayed";
}
