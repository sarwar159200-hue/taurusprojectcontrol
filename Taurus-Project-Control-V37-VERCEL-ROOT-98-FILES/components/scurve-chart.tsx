...
import { projectReportingWeek } from "@/lib/reporting-week";
...
export function ScurveChart({ points, granularity = "monthly" }: { points: CurvePoint[]; granularity?: "year" | "monthly" | "weekly" }) {
  ...
  const prepared = useMemo(() => controlledCumulativeCurve(points), [points]);
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const width = granularity === "weekly" ? 1600 : 1200;
  const height = granularity === "weekly" ? 470 : 448;
  const left = 66;
  const right = 34;
  const top = 34;
  const bottom = granularity === "weekly" ? 82 : 86;
  ...
  const monthBands = useMemo(() => {
    if (granularity !== "weekly" || !prepared.length) return [];
    const groups: Array<{ key: string; label: string; start: number; end: number }> = [];
    prepared.forEach((point, index) => {
      const date = new Date(`${point.date}T00:00:00Z`);
      const key = `${date.getUTCFullYear()}-${date.getUTCMonth()}`;
      const label = date.toLocaleDateString(dateLocale, { month: "short", year: "2-digit", timeZone: "UTC" });
      const last = groups.at(-1);
      if (last?.key === key) last.end = index;
      else groups.push({ key, label, start: index, end: index });
    });
    return groups;
  }, [prepared, granularity, dateLocale]);
      ...
      <div className="chart-fit-shell">
        <svg className="responsive-scurve-svg" preserveAspectRatio="xMidYMid meet" viewBox={`0 0 ${width} ${height}`} role="img" onMouseLeave={() => setHoveredIndex(null)}>
          ...
          {series.map(({ key, color, dash }) => (
            <path key={key} d={monotonePath(valuesFor(key))} fill="none" stroke={color} strokeWidth={key === "actual" ? 5 : 3.5} strokeDasharray={dash} strokeLinecap="round" strokeLinejoin="round" />
          ))}
          ...
          {granularity === "weekly" ? monthBands.map((band) => {
            const startX = x(band.start);
            const endX = x(band.end);
            return <g key={band.key} className="chart-month-band"><line x1={startX} x2={endX} y1={height - 24} y2={height - 24} /><text x={(startX + endX) / 2} y={height - 8} textAnchor="middle">{band.label}</text></g>;
          }) : null}
          ...
          {prepared.map((point, index) => {
            ...
            const reportingWeek = projectReportingWeek(point.date);
            const label = granularity === "weekly" && reportingWeek !== null ? `W${String(reportingWeek).padStart(2, "0")}` : dateLabel;
            ...
            return (
              <g key={`${point.date}-${index}`}>
                ...
                {showLabel ? <text x={x(index)} y={height - (granularity === "weekly" ? 40 : 32)} textAnchor={granularity === "weekly" ? "end" : "middle"} transform={granularity === "weekly" ? `rotate(-90 ${x(index)} ${height - 40})` : undefined} className="chart-axis-label chart-period-label">{label}</text> : null}
              </g>
            );
          })}
