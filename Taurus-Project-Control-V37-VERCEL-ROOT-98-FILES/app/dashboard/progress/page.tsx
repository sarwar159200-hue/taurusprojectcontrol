  ...
  return (
    <>
      ...
      <section className="kpi-grid five">
        ...
        <KpiCard label="SPI" value={performance.spi === null ? "—" : performance.spi.toFixed(2)} detail={signalLabel(performance.signal)} tone={performance.signal === "ahead" ? "green" : performance.signal === "on-plan" ? "green" : performance.signal === "slightly-behind" ? "amber" : "red"} />
        <KpiCard label="SV" value={performance.sv === null ? "—" : `${(performance.sv * 100).toFixed(2)}%`} detail="Actual minus baseline" tone={(performance.sv ?? 0) >= 0 ? "green" : "red"} />
      ...
      <section className="dashboard-grid lower-grid progress-notes">
        <article className="panel"><span className="eyebrow">FORECAST METHOD</span><h2>Baseline performance forecast</h2><p>Expected finish uses approved baseline duration divided by the same-date SPI (Actual ÷ Baseline). No current-plan values are used.</p></article>
        <article className="panel"><span className="eyebrow">STATUS THRESHOLDS</span><h2>Controlled schedule signal</h2><div className="thresholds"><span className="green-dot">Ahead &gt; 1.00</span><span className="green-dot">On schedule 0.98–1.00</span><span className="amber-dot">Slightly behind 0.95–&lt;0.98</span><span className="red-dot">Behind schedule &lt; 0.95</span></div></article>
      </section>
    </>
  );
}
