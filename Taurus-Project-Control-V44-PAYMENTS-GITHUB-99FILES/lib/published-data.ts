...
import { isDemoMode, isSupabaseConfigured, isSupabaseAdminConfigured } from "@/lib/config";
...
const PUBLISHED_COLUMNS = "id, project_id, progress_file_name, schedule_file_name, data_date, progress_analysis, schedule_analysis, progress_version_id, schedule_version_id, published_at, published_by";
...
const loadPublishedUpdateRow = cache(async () => {
  ...
  // The pointer row is identical for every authorized user. Cache it briefly
  // with the server client so tab-to-tab navigation does not repeatedly hit
  // Supabase. RLS session access remains the fallback when no server key exists.
  if (isSupabaseAdminConfigured) {
    try {
      const data = await unstable_cache(async () => {
        const admin = createAdminClient();
        const { data, error } = await admin.from("published_project_updates").select(PUBLISHED_COLUMNS).eq("project_id", projectId).maybeSingle();
        if (error) throw error;
        return data;
      }, ["taurus-published-pointer", projectId], { revalidate: 15 })();
      return data ? { data, projectId, supabase: sessionClient } : null;
    } catch {
      // Fall through to the signed-in RLS client.
    }
  }

  const { data, error } = await sessionClient.from("published_project_updates").select(PUBLISHED_COLUMNS).eq("project_id", projectId).maybeSingle();
