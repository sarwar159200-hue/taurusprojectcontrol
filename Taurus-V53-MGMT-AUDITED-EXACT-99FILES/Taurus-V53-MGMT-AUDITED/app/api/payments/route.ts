...
function supplierCurrentValues(workbook: ExcelJS.Workbook) {
  ...
  // GE and JC have no approved supplier-level contract variation in this workbook;
  // use the fixed-FX original values so the summary is on one consistent USD basis.
  current.GE = original.GE;
  if (spg) current.SPG = result(spg.getCell("P4")) || original.SPG; // workbook reflects the -USD 300,000 SPG change
  current.JC = original.JC;
