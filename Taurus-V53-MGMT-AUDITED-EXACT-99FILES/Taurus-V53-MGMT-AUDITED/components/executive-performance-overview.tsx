...
import { progressPerformance } from "@/lib/progress-metrics";
...
export function ExecutivePerformanceOverview({
  ...
  const disciplines = useMemo(
    () => {
      ...
      if (!detected.length && frequency === "monthly" && !progressSeries.length) return ["Overall"];
      return orderedUnique(detected, preferredDisciplines);
    },
    [frequencyPoints, frequency, progressSeries.length]
  );
  ...
  // Permanent single source of truth: status is derived from the SAME 2-decimal SPI
  // shown to the user. This prevents a label from disagreeing with the visible KPI.
  const displayedSpi = performance.spi === null || !Number.isFinite(performance.spi)
    ? null
    : Number(performance.spi.toFixed(2));
  // STRICT SPI STATUS RULE — single source of truth for BOTH text and styling.
  // Never infer status from SV, forecast dates, previous state, or a cached label.
  // The exact SPI displayed in the KPI is the SPI used here.
  const strictSpiStatus = displayedSpi === null
    ? { label: "Pending data", signal: "pending" as const }
    : displayedSpi < 1
      ? { label: "Behind Schedule", signal: "delayed" as const }
      : displayedSpi > 1
        ? { label: "Ahead of Schedule", signal: "ahead" as const }
        : { label: "On Schedule", signal: "on-plan" as const };
  const scheduleStatus = strictSpiStatus.label;
  const scheduleSignal = strictSpiStatus.signal;
  ...
  function changeFrequency(next: Frequency) {
    const nextPreferred = next === "weekly" ? ["Engineering", "Construction"] : [...DISCIPLINE_ORDER];
    const nextDisciplines = orderedUnique(
      progressSeries.filter((point) => point.frequency === next).map((point) => point.discipline),
      nextPreferred
    );
    setFrequency(next);
  ...
  return (
    <>
      <section className="executive-metric-grid">
        ...
        <KpiCard label="SPI" value={displayedSpi === null ? "—" : displayedSpi.toFixed(2)} detail={t(scheduleStatus)} tone={scheduleSignal === "ahead" ? "green" : scheduleSignal === "on-plan" ? "amber" : "red"} />
        <KpiCard label="Schedule variance" value={performance.sv === null ? "—" : `${performance.sv >= 0 ? "+" : ""}${(performance.sv * 100).toFixed(2)}%`} detail={performance.sv !== null && performance.sv >= 0 ? "Ahead of baseline" : "Behind baseline"} tone={(performance.sv ?? 0) >= 0 ? "green" : "red"} />
      ...
      <section className="dashboard-grid main-dashboard-grid executive-snapshot-main-grid">
        <article className="panel wide-panel executive-curve-panel">
          ...
          <div className="executive-curve-controls" aria-label={t("Discipline") + " / " + t("Sub-discipline")}>
            <label>
              ...
              <select disabled={!disciplines.length} value={discipline} onChange={(event) => { setDisciplineChoice(event.target.value); setSubdisciplineChoice("__all__"); }}>
        ...
        <article className="panel insight-panel">
          ...
          <div className={`signal-card signal-${scheduleSignal}`}>
            <span>{t("SCHEDULE STATUS")}</span>
            <strong>{t(scheduleStatus)}</strong>
            <p>SPI {displayedSpi === null ? "—" : displayedSpi.toFixed(2)} · SV {performance.sv === null ? "—" : `${performance.sv >= 0 ? "+" : ""}${(performance.sv * 100).toFixed(2)}%`}</p>
          </div>
