...
const projectNav: NavigationItem[] = [
  ...
  { href: "/dashboard/payments", icon: "$", label: "Payments", section: "payments" },
  { href: "/dashboard/android", icon: "↓", label: "Download Android App", section: "overview" },
  { href: "/dashboard/windows", icon: "⊞", label: "Download Windows App", section: "overview" }
];

const administrationNav: NavigationItem[] = [
  { href: "/dashboard/admin/imports", icon: "⚙", label: "Project Administrator", section: "imports", adminOnly: true },
...
export function AppShell({ user, children, progressDataDate, scheduleDataDate }: { user: CurrentUser; children: React.ReactNode; progressDataDate?: string | null; scheduleDataDate?: string | null }) {
  const { t } = useLanguage();
  const pathname = usePathname();
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const projectItems = projectNav.filter((item) => canAccessSection(user, item.section));
  const administrationItems = administrationNav.filter((item) =>
    canAccessSection(user, item.section) && (!item.adminOnly || user.role === "super_admin" || user.role === "project_admin")
  );

  // Warm every page the signed-in user can access. Next.js then keeps the
  // route payload in its client cache, making normal tab changes feel instant.
  useEffect(() => {
    const hrefs = [...projectItems, ...administrationItems].map((item) => item.href);
    const timer = window.setTimeout(() => {
      hrefs.forEach((href) => { if (href !== pathname) router.prefetch(href); });
    }, 150);
    return () => window.clearTimeout(timer);
  }, [pathname, router, user.role, user.permissions]);
      ...
      <div className="main-column">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <span className="live-indicator"><i /> {progressDataDate || scheduleDataDate ? t("Published data") : t("Awaiting first publish")}</span>
            <div className="date-badge date-badge-progress"><span>{t("PROGRESS DATA DATE")}</span><strong>{progressDataDate ?? t("Not published")}</strong></div>
            <div className="date-badge date-badge-schedule"><span>{t("SCHEDULE DATA DATE")}</span><strong>{scheduleDataDate ?? t("Not published")}</strong></div>
          </div>
        </header>
