# Taurus Project Control v18 — exact setup guide
...
- Root Directory: `taurusprojectcontrol-professional-v18`
...
If migrations 0001–0003 were installed previously, run all three of these in order:
...
Copy each complete file into Supabase → SQL Editor → New query and click **Run**. Migration 0005 must show the Taurus owner as an active `super_admin`. Migration 0006 must return `progress_version_id` and `schedule_version_id`. Migration 0007 adds and returns the MDR workflow fields `due_date`, `responsible_party` and `review_cycle_days`.

For a completely new database, run 0001, 0002, 0003, 0005, 0006 and 0007 in that order. Migration 0004 can remain installed; migrations 0005, 0006 and 0007 are idempotent.
