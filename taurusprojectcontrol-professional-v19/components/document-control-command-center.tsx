...
type KpiKey = "register" | "approved" | "under-review" | "overdue" | "due-next-7" | "avg-cycle";
...
export function DocumentControlCommandCenter({ documents, dataDate }: Props) {
  ...
  const kpiDefinitions: KpiDefinition[] = [
    ...
    { key: "overdue", icon: "△", label: "Overdue", value: overdueRows.length, detail: `${overdueDisciplines} ${t("disciplines")}`, tone: "red" },
    { key: "due-next-7", icon: "▣", label: "Due next 7 days", value: dueNextSeven.length, detail: t("Active contractual actions"), tone: "amber" },
    { key: "avg-cycle", icon: "↻", label: "Avg. review cycle", value: averageCycle ? averageCycle.toFixed(1) : "—", detail: averageCycle ? `${t("days")} · ${t("14-day target")}` : t("No completed cycle"), tone: "green" }
  ];
