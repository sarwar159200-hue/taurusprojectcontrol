...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: existing } = await admin.from("published_project_updates").select("*").eq("project_id", projectId).maybeSingle();
    ...
    const { error } = await admin.from("published_project_updates").upsert(row, { onConflict: "project_id" });
    if (error) throw error;
    await admin.from("audit_log").insert({ actor_id: user.id, event_type: "import.published", entity_type: "project_update", entity_id: projectId, project_id: projectId, details: { files: files.map((f) => f.name), actor_email: user.email } });
