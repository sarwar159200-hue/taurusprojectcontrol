...
- User deletion with self-deletion and super-administrator safeguards.
...
Then run `supabase/migrations/0003_live_excel_publishing.sql`. Its final result displays `default_project_id`. Copy that UUID for Vercel.
...
At first login, the user is redirected to create a new private password. Administrators can later edit permissions, issue another temporary password or remove the account. Administrators cannot view anyone's private password.
...
- User-management endpoints run only on the server using the Supabase service role.
...
The portal validates, analyzes and publishes replacement XLSX progress/MDR and schedule files that retain the supplied workbook formats. Dashboard snapshots are stored in Supabase and document hyperlinks are read from MDR column D. Binary workbook storage in OneDrive and native Primavera XER logic are not included in this release.
