...
export function scheduleSignal(spi: number | null): ScheduleSignal {
  ...
  // Standard SPI interpretation:
  // SPI > 1.00 = ahead of schedule
  // SPI = 1.00 = on schedule
  // SPI < 1.00 = behind schedule
  // Use a tiny tolerance only to avoid floating-point noise around exactly 1.00.
  const EPSILON = 1e-9;
  if (spi > 1.00 + EPSILON) return "ahead";
  if (Math.abs(spi - 1.00) <= EPSILON) return "on-plan";
  return "delayed";
}
...
export function signalLabel(signal: ScheduleSignal) {
  ...
  if (signal === "slightly-behind") return "Behind Schedule";
