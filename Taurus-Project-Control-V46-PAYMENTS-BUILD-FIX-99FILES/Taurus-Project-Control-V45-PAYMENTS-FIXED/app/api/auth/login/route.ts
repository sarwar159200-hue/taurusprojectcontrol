...
import { sendSecurityAlertEmail } from "@/lib/notifications/project-update-email";

export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as
    | { identifier?: string; password?: string; captchaToken?: string }
    | null;
  ...
  if (!identifier.includes("@")) {
    ...
    const { data, error: resolutionError } = await anonymous.rpc("resolve_login_email", {
      login_username: identifier
    });
    if (resolutionError) {
      return NextResponse.json({ error: "Username login requires migration 0005. Sign in with your email address for now." }, { status: 503 });
    }
    if (!data) {
  ...
  try {
    await sendSecurityAlertEmail({
      event: "login",
      actorEmail: email,
      actorName: signInData.user.user_metadata?.full_name ?? email.split("@")[0],
      detail: `Login method: ${identifier.includes("@") ? "email" : "username"}`
    });
  } catch {
    // Security-email delivery is best effort and must never block a valid login.
  }
