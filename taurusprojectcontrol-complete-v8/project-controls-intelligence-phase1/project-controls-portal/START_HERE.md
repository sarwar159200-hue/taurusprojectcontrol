# Taurus Project Control v8 — start here
...
Upload the folder named `project-controls-intelligence-phase1` from this package to the root of the GitHub repository.
...
## 2. Run the final Supabase repair

Migrations 0001–0003 must already exist. Then:

1. Open Supabase → SQL Editor → New query.
2. Open `project-controls-intelligence-phase1/project-controls-portal/supabase/migrations/0005_final_portal_repair.sql`.
3. Copy the entire SQL file, paste it in Supabase, and click **Run**.
4. Confirm the result shows `role = super_admin`, `is_active = true`, and the expected `default_project_id`.

Migration 0005 is safe to rerun. It includes the required final permissions, restores the named owner, repairs project membership, and enables username login.
...
Redeploy the latest commit without the old cache. Sign in, open **Import & Publish**, and select either workbook or both together. New filenames and updated dates/rows are accepted when the worksheet layout remains the same.
