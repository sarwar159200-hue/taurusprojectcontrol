...
`supabase/migrations/0005_final_portal_repair.sql`, in that order.
Migration 0005 displays `default_project_id`; copy that UUID for Vercel.

If migrations 0001–0004 are already installed, run only migration 0005. It is idempotent and repairs owner access, project membership, username login, activity logging and publishing permissions.
...
The expected repository layout remains:
...
Delete the old `project-controls-intelligence-phase1` folder from GitHub and upload the replacement folder from this package. Do not upload Excel, XER, `.env` or `.env.local` files.
...
- Authorized publishing, user administration and activity operations use the server-only Supabase secret so an incomplete optional RLS upgrade cannot break the portal.
...
The portal validates, analyzes and publishes replacement XLSX progress/MDR and schedule files that retain the supplied workbook formats. Dashboard snapshots are stored in Supabase and document hyperlinks are read from MDR column D. Monthly disciplines are Overall, Engineering, Procurement, Construction and Mobilization. Weekly curves are available for Engineering and Construction. Their exact workbook sub-disciplines are normalized to readable labels, all S-curves use cumulative values, and SPI/SV align planned and actual at the latest actual reporting point. Binary workbook storage in OneDrive and native Primavera XER logic are not included in this release.
