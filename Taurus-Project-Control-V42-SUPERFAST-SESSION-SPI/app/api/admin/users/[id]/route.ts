...
import { canCreateUsersByEmail, canGrantPaymentAccess } from "@/lib/roles";
...
async function authorize(): Promise<Authorization> {
  ...
  if (!canCreateUsersByEmail(actor.email) || !canAccessSection(actor, "user_access", "manage")) {
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!isSupabaseAdminConfigured) return NextResponse.json({ error: "SUPABASE_SERVICE_ROLE_KEY is required for user administration." }, { status: 503 });
  let admin;
  ...
  if (!canGrantPaymentAccess(actor.email)) {
    const existingPermissions = normalizePermissions((await supabase.from("profiles").select("section_permissions").eq("id", id).maybeSingle()).data?.section_permissions, target.role as AppRole);
    permissions.payments = existingPermissions.payments;
  }
  if (body.temporaryPassword) {
    const { error } = await admin.auth.admin.updateUserById(id, { password: body.temporaryPassword });
    if (error) return NextResponse.json({ error: friendlyAdminError(error) }, { status: 400 });
  }
  ...
  const { error: profileError } = await supabase.from("profiles").update(update).eq("id", id);
  if (profileError) return NextResponse.json({ error: profileError.message }, { status: 409 });

  const projectId = getDefaultProjectId();
  if (projectId) {
    await supabase.from("project_members").upsert({ project_id: projectId, user_id: id, role });
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!isSupabaseAdminConfigured) return NextResponse.json({ error: "SUPABASE_SERVICE_ROLE_KEY is required for user administration." }, { status: 503 });
  let supabase;
  try { supabase = createAdminClient(); } catch (error) {
    return NextResponse.json({ error: friendlyAdminError(error) }, { status: 503 });
  }
  const { data: target, error: targetError } = await supabase
    .from("profiles")
  ...
  await supabase.from("audit_log").insert({
    actor_id: actor.id,
    event_type: "user.access_removed",
    entity_type: "profile",
    entity_id: id,
    project_id: projectId,
