...
function UserEditor({ canManage, currentUserId, currentUserRole, user, onChanged }: {
  ...
  async function remove() {
    if (!window.confirm(`Remove ${user.fullName} (${user.email}) permanently?`)) return;
    setBusy(true);
      ...
      <div className="user-editor-body">
        ...
        {editable ? (
          <div className="user-action-row">
            ...
            <button className="danger-button" disabled={busy || user.id === currentUserId || user.role === "super_admin"} onClick={() => void remove()} type="button">Remove user</button>
          </div>
        ) : null}
      </div>
    </details>
