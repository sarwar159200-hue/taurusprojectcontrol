...
export function ExecutivePerformanceOverview({
  ...
  // Single source of truth for the visible schedule status. Never cache a label:
  // derive it from the currently selected curve's live SPI on every render.
  const scheduleStatus = performance.spi === null || !Number.isFinite(performance.spi)
    ? "Pending data"
    : performance.spi > 1
      ? "Ahead of Schedule"
      : performance.spi < 1
  ...
  return (
    <>
      <section className="executive-metric-grid">
        ...
        <KpiCard label="SPI" value={performance.spi === null ? "—" : performance.spi.toFixed(2)} detail={t(scheduleStatus)} tone={performance.signal === "ahead" ? "green" : performance.signal === "on-plan" ? "amber" : "red"} />
      ...
      <section className="dashboard-grid main-dashboard-grid executive-snapshot-main-grid">
        ...
        <article className="panel insight-panel">
          ...
          <div className={`signal-card signal-${performance.signal}`}>
            ...
            <p>SPI {performance.spi === null ? "—" : performance.spi.toFixed(2)} · SV {performance.sv === null ? "—" : `${performance.sv >= 0 ? "+" : ""}${(performance.sv * 100).toFixed(2)}%`}</p>
          </div>
