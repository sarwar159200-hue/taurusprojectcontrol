...
export async function POST(request: Request) {
  ...
  if (!canGrantPaymentAccess(actor.email)) permissions.payments = email === "saiwan.salih@taurusenergy.com" ? "manage" : "none";
  let admin;
  try {
  ...
  try {
    await sendSecurityAlertEmail({
      ...
      actorEmail: email,
      actorName: fullName,
      detail: `Created by ${actor.email}; role: ${role}`
    });
  } catch {
    // Account creation succeeds even if the notification provider is temporarily unavailable.
  }
