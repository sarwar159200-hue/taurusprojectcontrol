...
function parseWorkbook(workbook: ExcelJS.Workbook, fileName: string, userEmail: string): PaymentSnapshot {
  ...
  // Controlled overall project contract totals come directly from the Price Breakdown
  // Total row. Keep currencies separate; never add USD + EUR + PLN together.
  let projectTotalRow = 0;
  price.eachRow((row, rn) => { if (norm(row.getCell(1).value) === "total") projectTotalRow = rn; });
  const projectTotals = {
    USD: projectTotalRow ? result(price.getCell(`B${projectTotalRow}`)) : 0,
    EUR: projectTotalRow ? result(price.getCell(`C${projectTotalRow}`)) : 0,
    PLN: projectTotalRow ? result(price.getCell(`D${projectTotalRow}`)) : 0
  };
...
export async function GET() {
  ...
  dash.addRow([]);
  dash.addRow(["Overall Project Contract Totals", "USD", money(snapshot.projectTotals?.USD ?? 0), "EUR", money(snapshot.projectTotals?.EUR ?? 0), "PLN", money(snapshot.projectTotals?.PLN ?? 0)]);
