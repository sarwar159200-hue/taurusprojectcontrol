    ...
    <>
      <section className="panel progress-explorer premium-panel">
        ...
        <div className={`performance-signal performance-signal-${performance.signal}`}>
          ...
          <span>SPI {performance.spi === null ? "—" : performance.spi.toFixed(2)}</span>
        </div>
        <div className="selected-progress-kpis">
          ...
          <span><small>SPI</small><strong>{performance.spi === null ? "—" : performance.spi.toFixed(2)}</strong></span>
          <span><small>SV</small><strong>{performance.sv === null ? "—" : `${(performance.sv * 100).toFixed(2)}%`}</strong></span>
      ...
      {structure.length ? (
        <section className="panel progress-structure-panel">
          ...
          <div className="responsive-table"><table className="activity-table"><thead><tr><th>{t("Frequency")}</th><th>{t("Discipline")}</th><th>{t("Sub-discipline")}</th><th>{t("Baseline at actual")}</th><th>{t("Actual")}</th><th>SPI</th><th>{t("Expected finish")}</th></tr></thead><tbody>
            {structure.map((row) => <tr key={row.key}><td>{t(row.frequency === "weekly" ? "Weekly" : "Monthly")}</td><td><strong>{t(row.discipline)}</strong></td><td>{t(row.subdiscipline)}</td><td>{percent(row.performance.baseline)}</td><td>{percent(row.performance.actual)}</td><td>{row.performance.spi === null ? "—" : row.performance.spi.toFixed(2)}</td><td>{localizedDate(row.performance.expectedFinish)}</td></tr>)}
          </tbody></table></div>
        </section>
      ) : null}
    </>
