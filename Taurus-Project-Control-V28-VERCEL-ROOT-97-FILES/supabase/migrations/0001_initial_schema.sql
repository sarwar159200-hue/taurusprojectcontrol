...
create table public.profiles (
  ...
  section_permissions jsonb not null default '{"overview":"view","document_control":"view","progress":"view","schedule":"view","imports":"none","user_access":"none","activity_log":"none"}'::jsonb,
...
as $$
begin
  insert into public.profiles (id, email, username, full_name, role, section_permissions)
  values (
    new.id,
    ...
    case
      when lower(coalesce(new.email, '')) = 'sarwar.khalid@miranenergy.com' then 'super_admin'::public.app_role
      else 'viewer'::public.app_role
    end,
    case
      when lower(coalesce(new.email, '')) = 'sarwar.khalid@miranenergy.com'
        then '{"overview":"manage","document_control":"manage","progress":"manage","schedule":"manage","imports":"manage","user_access":"manage","activity_log":"manage"}'::jsonb
      else '{"overview":"view","document_control":"view","progress":"view","schedule":"view","imports":"none","user_access":"none","activity_log":"none"}'::jsonb
    end
  )
