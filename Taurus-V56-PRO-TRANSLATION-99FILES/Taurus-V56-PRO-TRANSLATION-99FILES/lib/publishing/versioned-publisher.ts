...
async function insertBatches(
  ...
  const batches = Array.from({ length: Math.ceil(rows.length / WRITE_BATCH_SIZE) }, (_, index) => {
    const start = index * WRITE_BATCH_SIZE;
    return { start, rows: rows.slice(start, start + WRITE_BATCH_SIZE) };
  });
  for (let index = 0; index < batches.length; index += WRITE_CONCURRENCY) {
    const window = batches.slice(index, index + WRITE_CONCURRENCY);
    await Promise.all(window.map(async (batch) => {
      const { error } = await supabase.from(table).insert(batch.rows);
      if (error) {
        throw new Error(`${label} could not be stored at rows ${batch.start + 1}–${Math.min(batch.start + WRITE_BATCH_SIZE, rows.length)}. ${migrationMessage(error)}`);
      }
    }));
  }
}
  ...
  try {
    ...
    if (analysis.kind === "progress") {
      const documents = (analysis.documents ?? []).map((row) => ({
        ...
        purpose: row.purpose,
        first_submission_date: row.firstSubmissionDate,
        ...
        review_cycle_days: row.reviewCycleDays,
        due_date: row.dueDate,
        responsible_party: row.responsibleParty,
        overdue_days: row.overdueDays,
        total_running_days: row.totalRunningDays,
        hold_by_taurus_days: row.holdByTaurusDays,
        hold_by_enka_days: row.holdByEnkaDays,
        delay_analysis: row.delayAnalysis,
        transmittal_no: row.transmittalNo,
        transmittal_date: row.transmittalDate,
