...
export function scheduleSignal(spi: number | null): ScheduleSignal {
  ...
  // Standard SPI status: > 1 ahead, = 1 on schedule, < 1 behind.
  // Round only for status tolerance so floating-point noise around 1.00
  // does not cause a false status; the displayed SPI remains the live value.
  const statusSpi = Math.round(spi * 100) / 100;
  if (statusSpi > 1.00) return "ahead";
  if (statusSpi === 1.00) return "on-plan";
  return "delayed";
}
...
export function signalLabel(signal: ScheduleSignal) {
  if (signal === "ahead") return "Ahead of Schedule";
  if (signal === "on-plan") return "On Schedule";
  if (signal === "slightly-behind") return "Slightly Behind Baseline";
  if (signal === "delayed") return "Behind Schedule";
  return "Pending data";
}
