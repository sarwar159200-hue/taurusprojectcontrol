...
export function DocumentControlCommandCenter({ documents, dataDate }: Props) {
  ...
  const kpiDefinitions: KpiDefinition[] = [
    ...
    { key: "due-next-7", icon: "▣", label: "Due next 7 days", value: dueNextSeven.length, detail: total ? `${((dueNextSeven.length / total) * 100).toFixed(1)}% ${t("of register")}` : "0%", tone: "amber" },
    { key: "on-hold", icon: "Ⅱ", label: "On hold", value: onHoldRows.length, detail: `${onHoldPercent.toFixed(1)}% ${t("of register")}`, tone: "amber" },
    { key: "avg-cycle", icon: "↻", label: "Avg. review cycle", value: averageCycle ? averageCycle.toFixed(1) : "—", detail: averageCycle ? `${cycleOverTargetPercent.toFixed(1)}% ${t("over 14-day target")}` : t("No completed cycle"), tone: cycleOverTargetPercent > 0 ? "red" : "green" }
  ];
